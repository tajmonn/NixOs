...
let
  ...
  caelestia-cli = pkgs.stdenv.mkDerivation rec {
    ...
    installPhase = ''
      ...
      find $out/share/caelestia-cli -name "*.py" -type f -exec sed -i '1s|^#!/bin/python3|#!${pkgs.python3.withPackages (ps: with ps; [ materialyoucolor pillow pyaudio numpy aubio ])}/bin/python3|' {} \;
      find $out/share/caelestia-cli -name "*.py" -type f -exec sed -i '1s|^#!/bin/python|#!${pkgs.python3.withPackages (ps: with ps; [ materialyoucolor pillow pyaudio numpy aubio ])}/bin/python|' {} \;
      find $out/share/caelestia-cli -name "*.py" -type f -exec sed -i '1s|^#!/usr/bin/env python3|#!${pkgs.python3.withPackages (ps: with ps; [ materialyoucolor pillow pyaudio numpy aubio ])}/bin/python3|' {} \;
      find $out/share/caelestia-cli -name "*.py" -type f -exec sed -i '1s|^#!/usr/bin/env python|#!${pkgs.python3.withPackages (ps: with ps; [ materialyoucolor pillow pyaudio numpy aubio ])}/bin/python|' {} \;
      ...
      makeWrapper ${pkgs.fish}/bin/fish $out/bin/caelestia \
        ...
        --prefix PATH : ${lib.makeBinPath (with pkgs; [
          ...
          foot
          jq
          (python313.withPackages (ps: with ps; [ materialyoucolor pillow pyaudio numpy aubio ]))
          grim
          wayfreeze
          wl-screenrec
          git
